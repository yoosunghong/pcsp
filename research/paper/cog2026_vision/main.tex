...
\begin{enumerate}
  ...
  \item \textbf{Deployment finding (Layer 3).} A frozen Layer-1 checkpoint
    ...
    (\S\ref{sec:ue5}).
    % TODO(T1.3, T1.4): scaling curve + persona-persistence figure.
...
% TODO(T1.3): scaling sweep at {8,16,32,64,96,128} agents with latency budget table.
% TODO(T1.4): long-horizon persona-persistence figure (4 personas x 30 in-game minutes, 1-min activity strip).
...
Table~\ref{tab:ue5_scaling} summarises the three validated scales and the
realtime budget under which they ran;
% TODO(T1.3): extend the sweep to {8, 96, 128} agents to draw a full scaling curve.
the inference budget itself is fixed by the engine wrapper.
The $0.5$\,s decision throttle plus exponential backoff on
\texttt{RecentFailureCount} caps inference at $\leq\!2$ calls/agent/s, so 64
agents generate $\leq\!128$ ONNX calls/s --- within the NNERuntimeORT CPU path's
headroom without async batching, and well below the
$16$--$33$\,ms per-frame wall budget of a 30--60\,FPS game loop.

\begin{table}[t]
  \centering
  \small
  \caption{%
    \textbf{UE5 scale and runtime characteristics.} Three validated scales on
    \texttt{Map\_PCSPDistrict\_M}; zone capacities tuned for the 64-agent
    target, so the 16-agent run is bottlenecked by category clustering. All
    runs share the same ONNX policy, the same $0.5$\,s decision throttle,
    and a $\geq\!60$\,FPS PIE wall-budget. Int./agent/min uses session
    \texttt{interaction\_complete} counts.}
  \label{tab:ue5_scaling}
  \setlength{\tabcolsep}{4pt}
  \begin{tabular}{rrrrrr}
    \toprule
    Agents & Dur.\,(s) & $n_{\text{int}}$ & Int./ag./min & Fail \% & Notes \\
    \midrule
    16 & 600 & 690   &  4.31 & 55.0 & Capacity-tuned for 64; 16 sees clustering only \\
    32 & 343 & 1{,}049 &  5.74 & 5.40 & Bottleneck zone capacity raised \\
    \textbf{64} & 483 & \textbf{2{,}438} & \textbf{4.73} & \textbf{1.70} & Stress test, 9/10 categories reached \\
    \bottomrule
  \end{tabular}
\end{table}
