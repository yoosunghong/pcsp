...
\begin{table}[t]
  ...
  \setlength{\tabcolsep}{3pt}
  \begin{tabular}{p{0.08\linewidth}p{0.20\linewidth}p{0.22\linewidth}p{0.14\linewidth}p{0.18\linewidth}c}
    \toprule
...
observations and an 8-action ontology. The current paper reports
\texttt{commons\_harvest\_\_open}; the same protocol is constructed to be
applied to additional substrates without algorithmic modification.
% TODO(T1.2): extend to clean_up and prisoners_dilemma_in_the_matrix__repeated, with held-out personas mirroring Layer 1, and cross-substrate persona transfer.
...
The Layer-1 training episode is $128$ environment steps; under the v3
action ontology a UE5 in-engine ``minute'' covers roughly $12$ policy
decisions per agent. To test whether persona identity persists over
horizons an order of magnitude longer than what the policy ever saw at
training time, we ran a long-horizon UE5 session
(\texttt{ue/cnzoi/Saved/PCSP/Logs/20260518\_121840}, $4{,}745$\,s
in standalone \texttt{-game}, $64$ agents, BT-abort rate
$2.5\%$---well below the contention regime of \S\ref{sec:ue5_contention})
and analysed the first $30$ in-game minutes
for four personas chosen \emph{a~priori} to maximise pairwise
...
(Rest-leaning), \texttt{p041} (Observe/Study mix), and \texttt{p058}
(Work-leaning). For each persona we bin every \texttt{decision} /
\texttt{interaction\_complete} event into $1$-minute windows and report
the dominant intent category per bin
...
\texttt{research/results/ue\_sessions/20260518\_121840/persona\_persistence.json}).
...
\begin{figure}[t]
  ...
  \caption{%
    ...
    personas over $30$ in-game minutes in UE5 (\emph{frozen} Layer-1
    checkpoint, $0.5$\,s decision throttle). Each row is one persona;
    each column is a $1$-minute bin coloured by the modal intent
    category. \texttt{p009} stays in \texttt{Rest} for all $30$ bins;
    \texttt{p058} stays in \texttt{Work} for $26/30$ bins ($87\%$);
    \texttt{p001} alternates between \texttt{Social} ($16$ bins) and
    \texttt{Rest} ($13$ bins); the high-entropy persona \texttt{p041}
    cycles through eight categories ($22$ category transitions). No
    persona ever ``forgets'' its dominant axis. The horizon is
    ${\sim}14\times$ a single Layer-1 training episode.}
...
is $1.00$ / $0.87$ / $0.53$ / $0.27$ for \texttt{p009} / \texttt{p058} /
\texttt{p001} / \texttt{p041} respectively---the persona ordering by
``focus'' is preserved end-to-end, and the most-focused persona stays
on its preferred axis for every single bin. (ii) \emph{Persistence is
persona-specific, not flat.} The high-entropy persona \texttt{p041}
visits eight distinct categories with $22$ transitions; the low-entropy
persona \texttt{p009} executes a single category run. The figure
therefore shows persona persistence \emph{without} flattening into
``every persona does its top thing forever''---which would be
...
persistence over a horizon $14\times$ the training episode is thus
purely a property of the persona embedding plus the InfoNCE-trained
conditioning manifold, not of any in-engine temporal smoothing.
...
budget liveable.
% TODO(T1.5): add a zone-capacity utilisation heatmap (per-zone occupancy timeseries) and a per-persona expressed-vs-preferred intent distribution chart.
