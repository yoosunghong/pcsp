...
\begin{enumerate}
  ...
  \item \textbf{Engine integration}: A UE5.5 deployment of the trained policy as
    a hybrid Behavior-Tree/ONNX stack reproduces the persona-conditioning
    ablation at 64 agents (1.7\% failure rate, $9$ active action categories),
    achieves $0.04\%$ failure rate zero-shot on held-out personas with
    inter-persona action dispersion $\rho = 0.368$ (vs.\ $0.383$ on train),
    \emph{and additionally reproduces the consistency-loss finding in-engine}:
    swapping the full checkpoint for the \texttt{no\_consist} checkpoint at
    fixed map and persona set leaves task throughput and reward higher
    ($4{,}005$ vs.\ $3{,}110$ interactions; reward $1{,}423.5$ vs.\ $1{,}079.8$)
    while matched-persona symmetric KL diverges to $1.79$\,nats, mirroring the
    Mini-Inzoi ``reward hides the failure'' pattern (\S\ref{sec:ue5}).
  \item \textbf{External validation on Melting Pot}: Applied to
    \texttt{commons\_harvest\_\_open} from \textit{Melting Pot
    2.4.0}~\cite{leibo2021meltingpot} with no algorithmic changes to the
    \S\ref{sec:method} method beyond a CNN observation front-end and a
    persona-agnostic team-reward potential required by the substrate, \PCSP
    produces persona-distinguishable behavior with mean pairwise action-KL
    $2.106\,[1.609,\,2.660]$ and trajectory$\to$persona retrieval top-3
    $0.936\,[0.879,\,0.993]$ across 5 seeds (chance $0.30$); the no-InfoNCE
    ablation confirms that the consistency loss remains load-bearing in this
    richer environment (\S\ref{sec:meltingpot}).
\end{enumerate}
...
% ---------------------------------------------------------------
\section{External Environment Expansion: Melting Pot}
\label{sec:meltingpot}

Mini-Inzoi v1--v3 isolate persona-conditioned control in a deliberately small
grid world. As a preliminary external check we apply the \S\ref{sec:method}
method---unchanged---to \texttt{commons\_harvest\_\_open} from \textit{Melting Pot
2.4.0}~\cite{leibo2021meltingpot}: a 7-player common-pool resource substrate with
...
\begin{table}[t]
  ...
  \small
  \caption{%
    \textbf{Melting Pot external check on \texttt{commons\_harvest\_\_open}.}
    Final-checkpoint metrics at $1\,\text{M}$ environment steps per seed.
    Mean pairwise action-KL (10 personas, 45 pairs, 64 random states) and
    in-distribution trajectory$\to$persona retrieval top-3 accuracy on the
    10-train vocabulary. Bootstrap 95\% CIs over seeds (10\,000 resamples).
    The no-InfoNCE ablation confirms the consistency loss remains load-bearing:
    KL alone is large but trajectories carry no persona information (top-3
    falls below chance), reproducing the v1--v3 finding.}
  \label{tab:mp_clean}
  \setlength{\tabcolsep}{4pt}
  \begin{tabular}{lrcc}
    \toprule
    Configuration & seeds & mean KL & top-3 \\
    \midrule
    Full \PCSP (\S\ref{sec:method})         & 5 & $2.106\,[1.609,\,2.660]$ & $0.936\,[0.879,\,0.993]$ \\
    \,\,$-\,$InfoNCE ($\lambda_{\text{IC}}\!=\!0$) & 1 & $5.819$ & $0.250$ \\
    chance (10 train personas)              & --& $0$       & $0.300$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{figure}[t]
  ...
  \caption{%
    ...
    longer encode \emph{which} persona produced them.}
  \label{fig:mp_clean}
\end{figure}

\paragraph{Takeaways.}
The \S\ref{sec:method} method---applied with no algorithmic modifications beyond
the CNN observation encoder and the persona-agnostic team-reward potential
required by the substrate---produces clearly persona-distinguishable behavior
in a multi-agent strategic substrate (Table~\ref{tab:mp_clean}). The no-InfoNCE
ablation reproduces the v1--v3 ablation finding in this richer environment: the
KL diversity loss alone drives policies apart but leaves no trajectory--persona
correspondence (top-3 falls to $0.250$, below chance), so the consistency loss
remains load-bearing. Multi-substrate generalization, held-out persona recovery,
and the per-seed projection-head landscape are deferred to a concurrent workshop
study~\cite{phase5report}.


% ---------------------------------------------------------------
\section{Engine Integration: UE5 Deployment}
\label{sec:ue5}
...
hybrid behaviour stack: \PCSP chooses semantic intents (e.g.\ \texttt{EatQuick},
\texttt{FocusedWork}, \texttt{LeisureOutdoor}), and Behaviour Tree, Blackboard,
EQS, AIController, and NavMesh execute them. The policy is exported to ONNX
...
We validated three target scales in PIE on a single workstation: 16 agents
(baseline, 690 interactions over 10\,min), 32 agents (1{,}049 interactions over
343\,s, 5.4\% failure rate after raising bottleneck zone capacity), and a
64-agent stress test (2{,}438 interactions over 483\,s at \textbf{1.7\% failure
rate}, $38.1$ interactions per agent, $9$ of $10$ active action categories
exercised). All 64-agent inference is real-time inside the game loop with a
$0.5$\,s decision throttle and urgency-driven preemption; the ONNX policy itself
remains the sub-frame component observed in the Python benchmark.
...
demand profile (Office, Hygiene). Result: $2{,}792$ interactions in $9.75$\,min,
...
has never seen, matching the Mini-Inzoi v3 zero-shot finding qualitatively.
...
We additionally test the Mini-Inzoi alignment metric inside the engine: for
...
construction). All four values are well below the Mini-Inzoi research-side
...
\textbf{Observability.}
Every agent emits a per-decision JSONL trace (action, $20$-d softmax logits,
...
The v2 experiment tests a larger grid and more agents, and the Melting Pot
external check (\S\ref{sec:meltingpot}) covers a single multi-agent
substrate (\texttt{commons\_harvest\_\_open}); commercial-scale multi-agent
validation, continuous physics, longer horizons, asynchronous events, and
richer social affordances remain untested.
...
\textbf{Engine--research alignment gap.}
The Mini-Inzoi persona--behavior alignment ($\rho \approx 0.73$) does
\emph{not} transfer to the engine directly. In the UE5 deployment the same
metric drops to $\rho \in [0.236, 0.257]$ on the full PCSP checkpoint
(\S\ref{sec:ue5}). The cause is structural: zone capacity contention and the
BT's failure-recovery decorators push agents toward what is reachable rather
than what their embedding most prefers. The persona-conditioning ablation
($\rho_{\text{intra}}$ collapse from $0.368$ to $0.990$) and the
consistency-loss replication remain visible inside the engine, but the
\emph{absolute} alignment number does not, and we have not yet tested whether
a less congested map narrows the gap.
