...
- [~] Report results separately for:
  ...
  - [x] unseen trait combinations (**v3 unseen_archetype**: full=0.203 [.16,.25] / 12.2× chance, concat=0.103 [.07,.14] / 6.2× chance, no_consist=0.013 (chance), no_diverse=0.163. Trained on `unseen_archetype_v3_train.json` (240, 12 archetypes), tested on 60 personas covering 3 held-out archetypes × 20 occs. Results: `results/pcsp_v3_archetype_zs/eval_archetype_summary.json`. See Decision Log 2026-05-09 for the split-family-specific FiLM/concat finding.)
  - [x] same occupation with different traits (covered by unseen_archetype_v3 — every test occupation is also seen during training, so success here means the policy can place a new personality at a known occupation context. Full=0.203, 12.2× chance.)
  - [~] same traits with different occupations (covered by unseen_occupation_v3 — every test archetype is seen during training. Full=0.170, 10.2× chance.)
...
- [~] Check tables/claims after any retraining or survey regeneration. (v3 retraining done; v3 zero-shot ablation table + observations paragraph added to `paper/cog2026_vision/main.tex` 2026-05-09. v1/v2 tables are unchanged. Revisit again after the unseen_archetype_v3 / unseen_combo_v3 retrains land.)
...
- [x] Re-run PCSP {no_consist, no_diverse, concat} + B1 + B3 on `train_240_v3` for a true zero-shot v3 ablation table. **Done 2026-05-09 in 1h 36min wall (5 modes via the `--include` flag added to `scripts/run_full_sweep_v3.py`).** Final rewards (train_240_v3, 300 iter): no_diverse=122.09, no_consist=118.38, concat=107.02, b3_sbert=106.85, full=104.08, b1_no_persona=100.29 — note that reward ordering scrambles vs in-distribution because the 240-train regime has fewer personas to fit, so the per-step persona-aligned bonus saturates differently across modes. **Zero-shot persona classification on `test_60_v3` (random=1.67%, n=300 trajectories per mode):** full=**0.170** [Wilson 95% CI 0.132–0.217] coherence 2.06; no_consist=**0.017** [0.007–0.038] coherence 1.07 (at chance — consistency-loss collapse confirmed at v3 zero-shot scale); no_diverse=**0.160** [0.123–0.206] coherence 2.05 (diversity loss marginal: Δ −0.01 vs full, ~within CI); **concat=0.283 [0.235–0.337] coherence 7.60** — concat conditioning *outperforms* FiLM in v3 zero-shot, reversing the v3 in-distribution pattern (full=0.290 > concat=0.117) but matching the v2 paper pattern (concat=0.027 ≥ full=0.023). Per-mode files: `results/pcsp_v3_zeroshot/{full,no_consist,no_diverse,concat}/eval_persona_classification_zs60.json`; combined: `results/pcsp_v3_zeroshot/eval_zs60_summary.json`. Eval driver: `scripts/run_eval_v3_zeroshot.py`.
...
- v3 zero-shot ablation table landed (full-paper main-track deliverable). Extended `scripts/run_full_sweep_v3.py` with `--personas_json`, `--pcsp_out`, `--baseline_out`, `--include` flags (additive, defaults reproduce the in-distribution sweep behavior). Launched 5 missing modes (no_consist, no_diverse, concat, b1, b3) on `train_240_v3.json`; full sweep finished in **5756.5s = 1.6h** wall (vs ~2h estimate; same threaded-trainer / GPU-underutilized regime as the in-dist sweep). New eval driver `scripts/run_eval_v3_zeroshot.py` evaluates the 4 PCSP modes against `test_60_v3.json`; 558.2s total wall on cuda. **Headline v3 zero-shot ablation table (random = 1.67%, n=300 trajectories):**
  ...
  | b1         | 100.29  | n/a         | —                | —         | —           |
  | b3_sbert   | 106.85  | n/a         | —                | —         | —           |
  **Three findings, one of which is new and surprising:**
  1. *Consistency-loss collapse holds at v3 zero-shot scale.* no_consist drops to 0.017 (at chance), with inter-trajectory cosine 0.89 (vs full 0.47) — trajectories from different personas become indistinguishable. The reward gap full→no_consist is **−14.30 reward but +0.153 accuracy** (full 104.08 acc 0.170 vs no_consist 118.38 acc 0.017). Removing the InfoNCE term lets the policy chase reward via persona-agnostic strategies, which is exactly what the v1 paper claim said it would. Zero-shot strengthens this story: in-distribution v3 had full vs no_consist at +1.04 reward / +0.273 acc; zero-shot v3 has **−14.30 reward / +0.153 acc** (now no_consist gets *higher* reward because it's not paying the consistency-loss tax, but its trajectories carry no recoverable persona signal). The "reward alone hides the failure mode" warning becomes even sharper.
  2. *Diversity loss is marginal and could be deemphasized in the paper.* full vs no_diverse: 0.170 vs 0.160 acc, Wilson CIs heavily overlap; coherence essentially identical (2.06 vs 2.05). Δreward is +18 in favor of no_diverse, but neither persona-recoverability nor trajectory-coherence is meaningfully harmed by removing the KL term at v3 zero-shot. Same pattern as the v1 paper (full 0.193 vs no_diverse 0.147), and same as v3 in-distribution (full 0.290 vs no_diverse 0.260). Recommendation: present diversity as the optional-add-on, not as a co-equal contribution.
  3. **(NEW) Concat conditioning *beats* FiLM in v3 zero-shot.** concat 0.283 (17.0× chance, coherence 7.60) vs full 0.170 (10.2×, coherence 2.06). Intra/inter cosine: concat 0.93/0.12 (very tight clusters, very separated centroids) vs full 0.96/0.47. **This reverses the v3 in-distribution finding** (full 0.290 > concat 0.117) **but matches v2 zero-shot** in the existing paper table (concat 0.027 ≥ full 0.023, both within noise). Plausible interpretation: FiLM's layer-wise persona gating gives the model more fitting capacity per persona, which helps in-distribution but increases the generalization gap when the test occupations were not seen — concat's input-only conditioning is more conservative and therefore generalizes better. The full-paper write-up needs to (a) report this honestly rather than pre-committing to "FiLM > concat", (b) discuss the in-dist→zero-shot reversal as an architectural-overfit observation, and (c) test whether the picture changes with longer rollouts, more episodes, or richer evaluation (the in-dist test set was 60 *random* personas; the zero-shot test set is 60 *structured* unseen-occupation personas, so there's an irreducible test-set-structure confound that should be addressed). **Paper-table implication:** the v3 zero-shot ablation row is now ready and is more interesting than originally expected — the concat-over-FiLM result is a publishable finding in its own right, not a footnote.
- Open follow-ups (not blocking the v3 ablation table itself):
  - Train a v3 PCSP-full + 3 ablations + B1 + B3 on `unseen_archetype_v3_train.json` and `unseen_combo_v3_train.json`, then run the same eval driver on the corresponding test files. ~2h × 2 = 4h compute. Closes the §4.3 lines 118-120 (unseen trait combinations / same-occupation-different-traits / same-traits-different-occupations).
  - Cross-check the concat>full zero-shot finding: the in-dist v3 eval *already* covers the same 60 IDs (since the in-dist sweep trained on all 300, including those 60), and the ranking there was full=0.290 > concat=0.117. So the in-dist→zero-shot reversal is presumptively a generalization-gap effect, not a test-set-structure artifact. Worth a focused note rather than another run; promoted to a paper-discussion item.
  - **Done 2026-05-09:** Add the v3 zero-shot ablation table to `paper/cog2026_vision/main.tex`. Added v3 entry to §V Experimental Setup, new Table~\ref{tab:results_v3} alongside the v1/v2 tables, updated the table reference line in §V Results, and added a "v3 reaffirms the consistency-loss finding and reveals a zero-shot architectural reversal" paragraph to §V Key Observations covering: (i) consistency-loss collapse replicates at v3 (full 0.170 vs no_consist 0.017, +14 reward gap from removing InfoNCE — sharpens the reward-vs-accuracy divergence), (ii) **NEW:** concat-over-FiLM reversal at v3 zero-shot (concat 0.283 vs full 0.170; reverses v3 in-dist 0.290>0.117) reported honestly as a generalization-gap observation, (iii) diversity-loss removal lands within the Wilson CI of full at v3 zero-shot, so the v1/v2 KL-collapse story does not directly translate to the zero-shot accuracy axis. PDF re-compiles clean at 7 pages, no undefined references. Caught and fixed one LaTeX error (`\bm{}` not in preamble; switched to `\mathbf{}` which is provided by `amsmath`).
- **Option B paper reframe committed (decision + edits 2026-05-09).** After the v3 zero-shot ablation landed with non-overlapping Wilson CIs (concat $[0.235, 0.337]$ vs full $[0.132, 0.217]$), Option A (re-run with more episodes to test "sample-size artifact" hypothesis) was rejected: doubling n only narrows CIs by $\sqrt{2}$, the 0.113 acc gap is statistically real, and the v2 paper already shows concat $\geq$ full at zero-shot. **Decision:** shift the paper's central claim from "FiLM > concat" to "**InfoNCE consistency loss is the load-bearing component; choice of conditioning architecture is secondary**." The concat>FiLM finding becomes a result (generalization-gap observation), not a problem. **Edits applied to `paper/cog2026_vision/main.tex`:**
  - Abstract: replaced "FiLM conditioning" with "neural persona conditioning" in the system summary; added a sentence stating that ablations across three scales identify the InfoNCE consistency objective—not the choice of FiLM vs concat—as load-bearing.
  - §I-C contributions item 2: replaced "FiLM conditioning" with "neural persona conditioning"; emphasized "PPO + InfoNCE consistency + KL diversity co-training objective".
  - §III-C subsection retitled "FiLM-Conditioned Shared Policy" → "Persona-Conditioned Shared Policy"; rationale text replaced — instead of claiming FiLM prevents persona-signal dilution, the paper now states explicitly that the conditioning mechanism is an implementation detail, points forward to v3 zero-shot as evidence that input-concatenation is competitive (and stronger on unseen occupations), and identifies the consistency objective as load-bearing across conditioning choices.
  - §IV Key Observations: complete reorder. New structure leads with (i) "**The consistency loss is the load-bearing component**" — single paragraph aggregating v1/v2/v3 evidence with explicit reward-axis-hides-failure framing; (ii) "**The conditioning architecture is secondary**" — frames the FiLM-vs-concat reversal as a generalization-gap knob rather than an architectural commitment, notes v1/v2 already showed FiLM and concat indistinguishable at zero-shot; (iii) Diversity loss now framed as "useful regularizer for KL spread, not co-equal contribution to zero-shot ID"; (iv) Spearman ρ stability (unchanged). Old "Three findings replicated in v2" framing dropped.
  - Figure 2 caption: removed the "diversity loss is the most critical ablation at both scales" claim (contradicted by v3 zero-shot accuracy); reframed to lead with consistency-loss collapse and to note that diversity loss's effect on v3 zero-shot accuracy is within Wilson CI of full.
  - §V Significance: "the same architectural decisions matter at both scales" → "the same design decisions—most centrally, the InfoNCE consistency objective—matter at both scales" (avoids ambiguity now that "architecture" is explicitly demoted).
  - §VII Conclusion: "behavioral regularization" expanded to "an InfoNCE-style consistency objective on trajectories", and the final message now explicitly states that "the choice of neural conditioning architecture is a tunable knob rather than a fundamental commitment" within this region of design space.
  - **PDF re-compiles clean at 7 pages.** Two pre-existing minor warnings (underfull hbox on intro paragraph, 8pt overfull on the LoRA equation in §III-B) remain — neither is introduced by this edit. No undefined references.
- **Why Option B is the stronger paper, not just the safer one:** the consistency-loss claim is supported by 3 independent scales (v1, v2, v3 in-dist, v3 zero-shot — actually 4 data points), with reward-axis-hides-failure being a counterintuitive finding that an "FiLM > concat" framing would have buried under an architecture story. The architecture observation is now itself a publishable finding (in-dist→zero-shot reversal as a generalization-gap signature), and it sharpens rather than dilutes the central message. The post-reframe paper has fewer load-bearing claims and more evidence per claim.
- **Korean human-eval recruitment kicked off in parallel (user).** Both rich and coarse survey variants ready at `data/human_eval/persona_identification_survey_ko{,_coarse}.{json,csv,md}`; automated baseline 29/30 = 96.7% (Wilson CI [0.83, 0.99]) provides the ceiling reference.
- **§4.3 unseen_archetype_v3 ablation table landed.** Sweep ran in **7042s = 1.96h** wall on `unseen_archetype_v3_train.json` (240 personas, 12 archetypes); 6 modes (full + 3 ablations + B1 + B3). Final rewards: pcsp_no_consist=105.85, pcsp_full=105.52, pcsp_concat=104.34, pcsp_no_diverse=99.80, b3_sbert=96.22, b1_no_persona=89.51. Results dirs: `results/pcsp_v3_archetype_zs/{full,no_consist,no_diverse,concat}/`, `results/baselines_v3_archetype_zs/{b1_no_persona,b3_sbert}/`. Eval driver `scripts/run_eval_v3_zeroshot.py` (with `--checkpoints_root` and `--test_personas` overrides) ran in ~9 min on `unseen_archetype_v3_test.json` (60 personas, 3 held-out archetypes × 20 occupations). **Headline archetype zero-shot ablation table (random=1.67%, n=300):**
  | mode       | reward  | ZS k-NN acc | Wilson 95% CI    | coherence | intra/inter |
  |:-----------|:--------|:------------|:-----------------|:----------|:------------|
  ...
  | b1         | 89.51   | n/a         | —                | —         | —           |
  | b3_sbert   | 96.22   | n/a         | —                | —         | —           |
- **MAJOR FINDING — concat>full reversal is split-family-specific, not universal.** Comparing the v3 zero-shot ablation across the two split families that have completed:
  | | unseen_OCCUPATION (4 occs held out) | unseen_ARCHETYPE (3 archs held out) |
  |:--|:--|:--|
  | full (FiLM) | 0.170 [.13, .22] | **0.203 [.16, .25]** |
  | concat | **0.283 [.24, .34]** | 0.103 [.07, .14] |
  | no_consist | 0.017* | 0.013* |
  | no_diverse | 0.160 | 0.163 |
  - **Wilson CIs are disjoint in BOTH directions.** Under unseen-occupation generalization, concat beats FiLM by 0.113 acc (CI gap: concat lower 0.235 vs full upper 0.217). Under unseen-archetype generalization, FiLM beats concat by 0.100 acc (CI gap: full lower 0.162 vs concat upper 0.143). Neither architecture dominates universally.
  - **Plausible mechanistic interpretation:** FiLM's per-layer gating uses persona embeddings to modulate hidden representations at every layer. When occupation is held out (unseen_occupation), the lower-layer gates may have over-fit to seen occupation features and break on novel occupations; concat conditioning, which routes the persona embedding only at the input layer, is more robust because the policy treats it as a key for nearest-neighbor lookup over seen-occupation prototypes. When archetype is held out (unseen_archetype), the personality dimension is the OOD axis; here FiLM's compositional layer-wise gating helps recombine seen-occupation features under novel personality conditioning, while concat's input-only routing has no analogous neighborhood to lean on. Speculative; the empirical pattern is what's load-bearing.
  - **Implication for Option B framing:** the paper's "consistency loss is load-bearing, architecture is secondary" claim is *strengthened* by this result. Consistency loss is the only component that universally fails across split families. Architecture choice is split-family-conditional — which makes "secondary" the right characterization (not "irrelevant"; the type of OOD shift matters). The previous paper text saying "input-concat conditioning is competitive with FiLM and in fact generalizes more strongly to unseen occupations" needs to be qualified: the "more strongly to unseen occupations" must be tagged as occupation-specific. **Pending:** combo sweep is currently running in bg (PID 445173, log `/tmp/pcsp_v3_combo_sweep.log`); will inspect for tie-breaker evidence. Paper §IV "The conditioning architecture is secondary" paragraph needs an additional sentence noting the archetype-split reversal.
- **Combo sweep launched in background.** `unseen_combo_v3_train.json` (240 personas, 15 archs × 18 occs after holding out 60 random cells). Only `pcsp_full` and `pcsp_no_consist` were already trained on this split (under `results/pcsp_v3_combo_zs/`); the remaining 4 modes (no_diverse, concat, b1, b3) launched 2026-05-09 ~20:33 KST via `scripts/run_full_sweep_v3.py --include pcsp:no_diverse pcsp:concat baseline:b1 baseline:b3`. Expected ~1h based on 4-mode-sweep extrapolation from archetype's 6-mode-1.96h timing. Eval to follow once sweep completes.
