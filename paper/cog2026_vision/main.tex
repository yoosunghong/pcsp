...
believability. The coarse-action pilot results indicate that evaluation traces
must expose the temporal, spatial, social, and stylistic details through which
humans recognize persona. Human studies should therefore compare coarse and rich
trajectory traces, report confidence and response time in addition to forced
choice accuracy, and track inter-rater agreement using established reliability
statistics such as Krippendorff's $\alpha$.
...
behavior. Pilot human-evaluation surveys showed that coarse labels such as
\texttt{read}, \texttt{rest}, and \texttt{eat} are difficult to interpret
without temporal, spatial, and social context. Mini-Inzoi v3 and the rich
rollout renderer address this partly, but finer social and stylistic action
semantics remain needed.
...
\textbf{No completed human evaluation.}
Zero-shot identification accuracy and Spearman $\rho$ are automated proxies.
Player judgments of persona fidelity and believability have not yet been
collected.
