...
\textbf{v3 reaffirms the consistency-loss finding and reveals a
zero-shot architectural reversal.}
At the richer 20-action ontology, removing the consistency loss again
collapses zero-shot accuracy to chance (0.017, Wilson CI $[.01,.04]$) while
\emph{raising} reward by 14 points (118.4 vs.\ 104.1). The reward--accuracy
divergence sharpens at v3: without InfoNCE, the policy chases reward via
persona-agnostic strategies, exactly as in v1/v2.
A new finding emerges in v3 zero-shot: \textbf{input-concat conditioning beats
FiLM} (concat 0.283 vs.\ full 0.170, $17.0\times$ vs.\ $10.2\times$ chance), and
its trajectory coherence is dramatically tighter (7.60 vs.\ 2.06).
Crucially, this reverses the v3 in-distribution ranking, where FiLM beats
concat (0.290 vs.\ 0.117): FiLM's layer-wise persona gating fits seen
personas more aggressively but generalizes worse to unseen occupations than
concat's input-only conditioning. We report this honestly as a generalization-gap
observation; revisiting the FiLM-vs-concat trade-off under longer rollouts and
broader test-set structure is left for future work.
Diversity-loss removal at v3 zero-shot is within Wilson CI of full
(0.160 vs.\ 0.170), so its v1/v2-style behavioral-collapse story (measured via
KL) does not directly translate to the zero-shot accuracy axis at v3 scale.
